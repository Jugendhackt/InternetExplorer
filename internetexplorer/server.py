...
file = open(path.join("internetexplorer", "server_config.json"))

configFileContent = json.loads(file.read())

shared_state = {"websocket": None}
...
def send_voice_input(input: str):
    send({"action": "input", "inputType": "voice", "inputText": input})
...
def send_browse_action_entry(actions):
    ...
    for action in actions:
        list.append({"type": action.type, "actionData": action.action_data, "result": action.result})

    send({"action": "action_entry", "actions": list})
